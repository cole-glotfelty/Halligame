...
%% Message structure:
%%      Server (from port):
%%            {valid, NewState} --> Send {state, NewState} to All Clients
%%            {invalid, {ClientPort, ErrorMessage}} --> Send {invalid, ErrorMessage} to ClientPort
...
handle_info({Port, {data, Data}}, {GameServerPort, ClientPorts}) ->
    case Port of
        GameServerPort ->   {MessageType, Message} = binary_to_term(Data),
                            case MessageType of
                                valid -> broadcast(ClientPorts, {state, Message});
                                invalid -> 
                                        {ClientPort, ErrorMessage} = Message,
                                        sendPortMessage(ClientPort, {invalid, ErrorMessage});
                                _ -> broadcast(ClientPorts, Message)
                            end,
                            {noreply, {GameServerPort, ClientPorts}};
        _ClientPort ->       {MessageType, Message} = binary_to_term(Data),
                            case MessageType of
                                event -> sendPortMessage(GameServerPort, {event, Message});
                                _ -> sendPortMessage(GameServerPort, {other, Message})
                            end,
                            {noreply, {GameServerPort, ClientPorts}}
    end.
