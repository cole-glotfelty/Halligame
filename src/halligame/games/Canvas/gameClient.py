...
class Client(ClientSuper):
    ...
    def __initializeColors(self):
        ...
        self.__screen.addColorPair("white", "colorSwatchBackground", 
                                   "colorSwatchBackground")

        self.__screen.addColor(242, 182, 61, "currColorHighlight")
        self.__screen.addColorPair("white", "currColorHighlight", 
                                   "currColorHighlight")

        self.__screen.setStyle("black") # set background to black

    def __defineMouseRegions(self):
        self.__screen.addClickableRegion(self.__boardVOffset, self.__boardHOffset, self.__boardHeight, self.__boardWidth, "board")
        ...
        swatchBoxDraw = ((" " * (colorBoxWidth + 4)) + "\n") * (((colorBoxHeight * len(self.__colors)) + (colorBoxVSeparator * (len(self.__colors) - 1)) + 2))

        self.__screen.write(self.__boardVOffset - 1, colorBoxCol - 2, swatchBoxDraw, "colorSwatchBackground")

        for i in range(len(self.__colors)):
            colorBoxRow = self.__boardVOffset + (i * (colorBoxHeight + colorBoxVSeparator))
            self.__screen.addClickableRegion(colorBoxRow, colorBoxCol, 
                                             colorBoxHeight, colorBoxWidth, 
                                             self.__colors[i])
            self.__screen.write(colorBoxRow, colorBoxCol, colorBoxDraw, 
                                self.__colors[i])
            
        currColorBox = ((" " * (colorBoxWidth + 4) + "\n") * (colorBoxHeight + 2))
        self.__currColorTopRow = self.__boardVOffset + (len(self.__colors) * (colorBoxHeight + colorBoxVSeparator)) + 6
        self.__currColorLeftCol = colorBoxCol
        self.__screen.write(self.__currColorTopRow - 1, self.__currColorLeftCol - 2, currColorBox, "currColorHighlight")
    ...
    def __updateCurrColor(self, color):
        ...
        self.__screen.write(self.__currColorTopRow, self.__currColorLeftCol, colorBoxDraw, color)
        self.__screen.refresh()
